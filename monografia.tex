...
\chapter{Resultados obtidos}\label{cap:conclusao}
	...
	Isso se deve principalmente ao tamanho dos cabeçalhos que são transmitidos em cada requisição AJAX. Na Tabela \ref{tab:dados} nota-se que enquanto uma mensagem de SEND da implementação AJAX possui em média 772,25 bytes, as mensagens de SEND da implementação Websocket possuem em média 113,78 bytes. Ambas possuem uma variação pequena que pode ser confirmado pelos seus desvios padrão 29,02 e 29,41 respectivamente e que pode ser explicada pelo tamanho do conteúdo randômico que foi utilizado nas implementações dos \textit{scripts} de teste. Com esses dados, calculamos que a implementação Ajax trafegou um total de 45,37 Mb de dados, enquanto que a implementação em WebSockets trafegou 36,39 Mb. No entanto, é importante ressaltar que essa diferença entre as duas implementações tende a se tornar menos significativa conforme o tamanho do conteúdo das mensagens aumenta.
